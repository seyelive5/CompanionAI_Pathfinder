    ...
    public class ExecutionResult
    {
        ...
        public static ExecutionResult CastAbility(AbilityData ability, TargetWrapper target)
        {
            return new ExecutionResult
            {
                ...
                Ability = ability,
                Target = target,
                Reason = $"Cast {ability?.Name} on {target?.Unit?.CharacterName ?? "point"}"
            };
        }

        /// <summary>
        /// MultiTarget 능력 시전 명령
        /// 2개 이상의 타겟이 필요한 능력에 사용
        /// </summary>
        ...
        /// ★ v0.2.22: 기본 공격 명령
        /// </summary>
        public static ExecutionResult BasicAttack(string targetName)
        {
            return new ExecutionResult
            {
                Type = ResultType.CastAbility,  // 동일한 결과 타입 사용
                Reason = $"Basic attack on {targetName}"
            };
        }
